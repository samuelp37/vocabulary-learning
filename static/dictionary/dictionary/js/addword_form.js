...
function choose(elt,prefix_id,word,gender,lng){

	form_adding_word = elt.closest(".adding_word") // Getting the div surrounding the subform to "Add a word"
	list_words = form_adding_word.getElementsByTagName("ul")[0];

	document.getElementById(String(prefix_id)+"-word").value = String(word);
	document.getElementById(String(prefix_id)+"-gender").value = gender;
	document.getElementById(String(prefix_id)+"-language").value = lng;
